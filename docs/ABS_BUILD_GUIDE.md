...
| Item | Where | Fix |
|---|---|---|
| Password reset | ✅ shipped (PR #6) — `POST /auth/password/reset/` (enumeration-safe: returns the same response whether or not the email exists) issues a `TimestampSigner`-signed token via [`apps/core/signing.py`](../backend/apps/core/signing.py) that embeds the user's password-hash fingerprint, emails a one-shot link, and the token expires after 1 h. `POST /auth/password/reset/confirm/` rotates the password and blacklists every outstanding refresh token. Both endpoints are throttled at `5/hour` per IP via the `password_reset` scope. Frontend pages: [`/auth/forgot-password`](../src/app/auth/forgot-password/page.tsx), [`/auth/reset-password/[token]`](../src/app/auth/reset-password/[token]/page.tsx). |
