...
| Color contrast audited | ✅ shipped (PRs #19a + #19b) | Two-layer assurance: (1) [`src/__tests__/axe-sweep.test.tsx`](../src/__tests__/axe-sweep.test.tsx) renders LoginForm, ForgotPasswordForm, ResetPasswordForm, HomePageClient (CMS + fallback), ComparePageClient, AccountPanel and runs `axe-core` over each — 0 critical/serious violations. (2) [`src/__tests__/color-contrast.test.ts`](../src/__tests__/color-contrast.test.ts) computes WCAG 2.1 ratios programmatically for every documented foreground/background pair (jsdom can't compute contrast, so the policy runs as a separate gate). The Signal Orange flag was real: `accent-500 (#F97316)` on warm-white only hits 2.2:1 — well under AA. Fix: introduced `accent-700 (#B5441E)` (5.3:1 on warm-white) for text-on-light contexts, pinned the policy in [`globals.css`](../src/app/globals.css) (Signal Orange = decorative; `accent-700` = text). PR #19a established the policy, tests, and token; PR #19b completed the class-swap sweep across the visible site. One discovered bug fixed: `aria-label` on a div without role on TestimonialCarousel star rating → added `role="img"`. |
...
| Use CMS-provided variants via Next.js Image | ✅ shipped (helper landed in PR for variant picker) | New [`pickMediaVariant(asset, slot)`](../src/lib/media.ts) helper returns `{src, srcSet, width, height, alt}` for an explicit slot (`hero`/`thumbnail`/`inline`/`avatar`). Per-slot fallback chain favours the smallest variant that still satisfies the rendered size, prefers webp as the primary `src`, and emits a `srcSet` so the browser picks based on the `<Image sizes={...}>` spec. 13 vitest cases pin the contract (fallback chain, original-as-last-resort, alt echo, null-safe). Existing `<Image>` consumers can adopt it incrementally — call sites that pass `asset.file` directly still work, but `pickMediaVariant` is now the recommended call when an asset has variants. |
...
| Email verification | ✅ shipped (PR #7, PR #20 sensitive-action gating) | `User.email_verified_at` added; register auto-sends a signed link (24 h TTL, fingerprint binds `email + email_verified_at` so the token is single-use). `POST /auth/email/verify/request/` (auth-required, 5/h throttle) re-sends; `POST /auth/email/verify/confirm/` consumes the token, flips `email_verified_at`, writes an audit row. Frontend pages: [`/auth/verify-email/[token]`](../src/app/auth/verify-email/[token]/page.tsx) (auto-confirms on mount), [`/auth/verify-email`](../src/app/auth/verify-email/page.tsx) (auth re-send). `User.email_verified` is exposed via `/auth/me/`. **Sensitive-action gating** (PR #20): new [`apps.accounts.permissions.IsEmailVerified`](../backend/apps/accounts/permissions.py) — applied to `MeDeleteView`, `TrialSignupCancelView`, and (via `AnonymousOrEmailVerified`) `TrialSignupCreateView` so anonymous marketing-form submissions stay open but authenticated unverified users get 403 with a curated message. Login, `/me/`, and the verification-resend endpoint stay open (the chicken-and-egg path that unblocks the user). New [`EmailVerificationBanner`](../src/components/auth/EmailVerificationBanner.tsx) lives in the portal layout and lets unverified users one-click resend. 8 backend tests (`test_email_verified_gate.py`) + 5 frontend tests. |
...
| Data-retention notes + automated sweep | ✅ shipped (PR #10 + retention-sweep PR) — Privacy Policy `/privacy` § 4 and `docs/COMPLIANCE.md` document the 24-month retention window. The automated enforcement is now in place: new [`apps.accounts.tasks.sweep_inactive_accounts`](../backend/apps/accounts/tasks.py) runs daily at 03:15 UTC via Celery Beat (`CELERY_BEAT_SCHEDULE`) and soft-deletes accounts whose `last_login` (or `created_at` for never-logged-in users) is past `settings.ACCOUNT_RETENTION_DAYS` (default 720 days = ~24 months). Skips admin/staff and already-deleted rows. Continues past per-user failures so one bad row doesn't abort the sweep. 6 backend tests pin selection logic + admin-skip + idempotency. |
...
- [x] `python manage.py check --deploy` returns clean on production settings — wired into CI as the `Backend · manage.py check --deploy` job. Uses placeholder-but-real-shaped secrets (50-char Django key, 32-char JWT, etc.) that satisfy `production.py`'s required-secret gate without granting any real access. `--fail-level WARNING` upgrades any deploy advisory to a hard fail so a future regression that flips, e.g., `SECURE_SSL_REDIRECT` off cannot pass CI silently.
