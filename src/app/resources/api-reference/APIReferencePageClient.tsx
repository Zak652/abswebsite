...
const FALLBACK_ENDPOINTS: FallbackGroup[] = [
    {
        group: "Products",
...
function cmsToGroups(cmsGroups: APIEndpointGroupData[]): FallbackGroup[] {
    return cmsGroups.map((g) => ({
        group: g.name,
        badgeClass: g.badge_class,
...
function CopyButton({
    ...
    return (
        <button
            onClick={handleCopy}
            className={`inline-flex items-center gap-1 px-2 py-1 rounded-lg text-xs font-medium transition-all shrink-0 cursor-pointer ${copied
...
interface APIReferencePageClientProps {
    cmsGroups: APIEndpointGroupData[];
}

export default function APIReferencePageClient({
    cmsGroups,
}: APIReferencePageClientProps) {
    const groups =
        cmsGroups.length > 0 ? cmsToGroups(cmsGroups) : FALLBACK_ENDPOINTS;
            ...
            <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div className="bg-white rounded-3xl border border-neutral-100 p-8 shadow-sm mb-6">
                    ...
                    <p className="text-sm text-primary-900/60 mb-5">
                        ...
                        token issued from your Arcplus admin console. Tokens are
                        scoped per organisation and can be revoked at any time
                        from Settings.
                    </p>
                        ...
                        >
                            ...
                            <div className="divide-y divide-neutral-100">
                                {group.endpoints.map((ep) => (
                                    ...
                                    >
                                        <span
                                            className={`shrink-0 text-[11px] font-bold font-mono px-2.5 py-1 rounded-lg min-w-[52px] text-center ${METHOD_STYLES[ep.method]
                                                }`}
                                        >
                                            {ep.method}
                                        </span>
                ...
                <div className="bg-white rounded-3xl border border-neutral-100 shadow-sm p-8 mb-6">
                    <div className="flex items-center justify-between mb-3">
                        ...
                        <button
                            ...
                            className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-all cursor-pointer ${codeBlockCopied
                                    ? "bg-[var(--color-success-light)] text-[var(--color-success)]"
                                    : "bg-neutral-100 text-neutral-600 hover:bg-neutral-200"
                                }`}
                        >
                ...
                <div className="bg-[#F5F3FF] rounded-3xl border border-[#EDE9FE] p-8 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                    <div>
                        ...
                        <p className="text-sm text-primary-900/60 mt-1">
                            API tokens are available on Growth and Enterprise
                            tiers. Upgrade your plan to unlock programmatic
                            access to Arcplus.
                        </p>
                    </div>
                    <Link
